...
import { addRxPlugin, getSchemaByObjectPath, newRxError } from 'rxdb';
import type {
	ById,
	WithDeleted,
	RxCollection,
...
export class RxFirestoreReplicationState<RxDocType> extends RxReplicationState<
	RxDocType,
	FirestoreCheckpointType
> {
	constructor(
...
export function replicateFirestore<RxDocType>(
	...
	const pullStream$: Subject<RxReplicationPullStreamItem<RxDocType, FirestoreCheckpointType>> =
		new Subject();
	let replicationPrimitivesPull:
		| ReplicationPullOptions<RxDocType, FirestoreCheckpointType>
		| undefined;
	options.live = typeof options.live === 'undefined' ? true : options.live;
	options.waitForLeadership =
		typeof options.waitForLeadership === 'undefined' ? true : options.waitForLeadership;
	const serverTimestampField =
		typeof options.serverTimestampField === 'undefined'
			? 'serverTimestamp'
			: options.serverTimestampField;
	...
	const pullFilters = options.pull?.filter !== undefined ? toArray(options.pull.filter) : [];

	const pullQuery = query(options.firestore.collection, ...pullFilters);

	if (options.pull) {
		replicationPrimitivesPull = {
			async handler(lastPulledCheckpoint: FirestoreCheckpointType | undefined, batchSize: number) {
				let newerQuery: ReturnType<typeof query>;
				let sameTimeQuery: ReturnType<typeof query> | undefined;
				if (lastPulledCheckpoint) {
					console.debug(lastPulledCheckpoint.serverTimestamp);
				...
				while (mustsReRun) {
					...
					await runTransaction(options.firestore.database, async (_tx) => {
						...
						try {
							...
							console.debug('newerQueryResult', newerQueryResult.docs.length);
							console.debug('sameTimeQueryResult', sameTimeQueryResult?.docs.length);
							if (
				...
				const documents: WithDeleted<RxDocType>[] = useDocs.map((row) =>
					firestoreRowToDocData(serverTimestampField, primaryPath, row)
				);
